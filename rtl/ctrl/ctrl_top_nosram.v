...
/*
DESCRIPTION
This is minimig-de0_cv's control sub-module. It contains a PLL for clock generation, a reset generator,
OR1200 CPU, and a register slave, which contains reset controls, imer, UART serial transmitter and a SPI master.
...
Slaves:  There are three slaves, ROM, SDRAM and REGS.
...
The CPU boots from address 0x000004 (internal ROM).
*/

module ctrl_top_nosram (
  // system
  input  wire           clk_in,
  input  wire           rst_ext,
  output wire           clk_out,
  ...
  // Host RAM interface
  output wire [ 24-1:0] ram_adr,
  output wire           ram_cs,
  output wire           ram_we,
  output wire [  4-1:0] ram_sel,
  output wire [ 32-1:0] ram_dat_w,
  input  wire [ 32-1:0] ram_dat_r,
  input  wire           ram_ack,
  input  wire           ram_err,
  // UART
...
assign rst = rst_out || sys_status[0];
...
ctrl_rst ctrl_rst (
  ...
  .rst        (rst_out    )   // reset signal output, active high
);
