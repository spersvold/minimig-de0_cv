...
#define OSD_CMD_RST       0x08      // OSD reset command
#define OSD_CMD_CLK       0x18
#define OSD_CMD_OSD       0x28
#define OSD_CMD_CHIP      0x04      // OSD settings: chipset config
#define OSD_CMD_CPU       0x14      // OSD settings: CPU config
#define OSD_CMD_MEM       0x24      // OSD settings: memory config
#define OSD_CMD_VID       0x34      // OSD settings: video config
#define OSD_CMD_FLP       0x44      // OSD settings: floppy config
#define OSD_CMD_HDD       0x54      // OSD enable HDD command
...
#define spi16(parm) do {                        \
        SPI((parm) >> 8);                       \
        SPI((parm) >> 0);                       \
    } while(0)

#define spi24(parm) do {                        \
        SPI((parm) >> 16);                      \
        SPI((parm) >> 8);                       \
        SPI((parm) >> 0);                       \
    } while(0)

#define spi32(parm) do {                        \
        SPI((parm) >> 24);                      \
        SPI((parm) >> 16);                      \
        SPI((parm) >> 8);                       \
        SPI((parm) >> 0);                       \
...
#define spi_n(value, cnt) do {                  \
        typeof(cnt) _cnt = (cnt);               \
        while(_cnt--)                           \
            SPI(value);                         \
    } while(0)

#define spi_osd_cmd_cont(cmd) do {              \
        EnableOsd();                            \
        SPI(cmd);                               \
    } while(0)

#define spi_osd_cmd(cmd) do {                   \
    spi_osd_cmd_cont(cmd);                      \
    DisableOsd();                               \
    SPIN(); SPIN(); SPIN(); SPIN();             \
    } while(0)

#define spi_osd_cmd8_cont(cmd, parm) do {       \
        EnableOsd();                            \
        SPI(cmd);                               \
        SPI(parm);                              \
    } while(0)

#define spi_osd_cmd8(cmd, parm) do {            \
        spi_osd_cmd8_cont(cmd, parm);           \
        DisableOsd();                           \
        SPIN(); SPIN(); SPIN(); SPIN();         \
    } while(0)

#define spi_osd_cmd32_cont(cmd, parm) do {      \
        EnableOsd();                            \
        SPI(cmd);                               \
        spi32(parm);                            \
    } while(0)

#define spi_osd_cmd32(cmd, parm) do {           \
        spi_osd_cmd32_cont(cmd, parm);          \
        DisableOsd();                           \
        SPIN(); SPIN(); SPIN(); SPIN();         \
    } while(0)

#define spi_osd_cmd32le_cont(cmd, parm) do {    \
        EnableOsd();                            \
        SPI(cmd);                               \
        spi32le(parm);                          \
    } while(0)

#define spi_osd_cmd32le(cmd, parm) do {         \
