...
module paula_floppy
  (
   ...
   input  wire        ntsc,                    // ntsc mode
   input  wire        sof,                     // start of frame
   input  wire        enable,                  // dma enable
   ...
   output wire        dmal,                    // dma request output
   output wire        dmas,                    // dma special output
   ...
   output wire        syncint,                 // disk syncword found
   input  wire        wordsync,                // wordsync enable
   ...
   output wire        disk_led,                // disk activity LED, active when DMA is on
   input  wire [ 1:0] floppy_drives,           // floppy drive number

   input  wire        direct_scs,              // enables direct data transfer from SD card
   input  wire        direct_sdi,              // data line from SD card
   input  wire        hdd_cmd_req,             // HDD requests service (command register has been written)
   input  wire        hdd_dat_req,             // HDD requests data tansfer
   output wire [ 2:0] hdd_addr,                // task file register address
   output wire [15:0] hdd_data_out,            // data from HDD to HDC
   input  wire [15:0] hdd_data_in,             // data from HDC to HDD
   output wire        hdd_wr,                  // task file register write strobe
   output wire        hdd_status_wr,           // status register write strobe (MCU->HDD)
   output wire        hdd_data_wr,             // data write strobe
   output wire        hdd_data_rd,             // data read strobe
   // fifo / track display
   ...
   localparam DSKBYTR = 9'h01a;
   localparam DSKDAT  = 9'h026;
   localparam DSKDATR = 9'h008;
   localparam DSKSYNC = 9'h07e;
   localparam DSKLEN  = 9'h024;
   ...
   // there is a possibility that SCS2 is inactive before rx_flag is generated, depends on how fast the CS2 is deaserted after sending the last data bit
   ...
   //disk states
   localparam DISKDMA_IDLE   = 2'b00;
   localparam DISKDMA_ACTIVE = 2'b10;
   localparam DISKDMA_INT    = 2'b11;
   ...
   always @(posedge clk)
     if (clk7_en) begin
        ...
        else if (rx_flag && rx_cnt==0) begin
   ...
   always @(*) begin
      case(dskstate)
        DISKDMA_IDLE: begin//disk is present in flash drive
           trackrd = 0;
           trackwr = 0;
           dmaon = 0;
           blckint = 0;
        ...
        DISKDMA_ACTIVE: begin//do disk dma operation
        ...
        DISKDMA_INT:begin//generate disk dma completed (DSKBLK) interrupt
           trackrd = 0;
           trackwr = 0;
           dmaon = 0;
           blckint = 1;
