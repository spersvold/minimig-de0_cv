...
#ifndef STAGE1
unsigned char fattype = 0;              // volume format
#endif
...
#ifdef STAGE1
    if (strncmp((const char*)&sector_buffer[0x52], "FAT32   ",8)==0) // check for FAT16
    {
        fat32=1;
    }
    else if (strncmp((const char*)&sector_buffer[0x36], "FAT16   ",8)!=0) // check for FAT32
    {
...
unsigned char FileOpen(fileTYPE *file, const char *name)
{
    ...
    else // root directory
    {
        ...
        nEntries = fat32 ?  (unsigned long)cluster_size << 4 : root_directory_size << 4; // 16 entries per sector
    }
#else
    buffered_fat_index=-1;
            ...
            if (pEntry->Name[0] != SLOT_EMPTY && pEntry->Name[0] != SLOT_DELETED) // valid entry??
            {
                if (!(pEntry->Attributes & (ATTR_VOLUME | ATTR_DIRECTORY))) // not a volume nor directory
                {
                    if (strncmp((const char*)pEntry->Name, name, 11) == 0)
                    {
                        ...
                        file->start_cluster = file->cluster;
...
char ScanDirectory(long mode, char *extension, unsigned char options)
{
    DEBUG_FUNC_IN(DEBUG_F_FAT | DEBUG_L2);
                    ...
                    if (!(pEntry->Attributes & (ATTR_VOLUME | ATTR_HIDDEN)) && (pEntry->Name[0] != '.' || pEntry->Name[1] != ' ')) // if not VOLUME label (also filter current directory entry)
                    {
                        if ((extension[0] == '*') || (strncmp((const char*)&pEntry->Name[8], extension, 3) == 0) || (options & SCAN_DIR && pEntry->Attributes & ATTR_DIRECTORY))
                        {
                            ...
                            { // find a dir entry with given cluster number and store it in the buffer
                                if (pEntry->StartCluster + (fat32 ? (unsigned long)(pEntry->HighCluster & 0x0FFF) << 16 : 0) == iPreviousDirectory)
...
int LoadFile(const char *fn, unsigned char *buf)
{
    if(FileOpen(&file,fn))
    {
        ...
        while(c<imgsize)
        {
            ...
            {
                file.cluster=GetFATLink(file.cluster);
            }
